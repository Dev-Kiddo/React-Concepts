...
const AgeVerification = () => {
  ...
  return (
    <div>
      {userState?.age && isAlert == true && (
        <div className={`alert ${userState.age >= 18 ? "success" : "failure"}`}>
          {userState.age >= 18 ? "You are eligible" : "You are not eligible"}
        </div>
      )}
      ...
      <div>
        ...
        <input
          type="number"
          onChange={(event) => {
            setUserState({ ...userState, age: event.target.value });
            setAlert(false);
          }}
        />
      </div>
